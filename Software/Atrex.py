...
class Atrex(QMainWindow):
    ...
    def __init__(self):
        ...
        self.ui = loadUi("uiMainWin.ui", self)
        self.myLogWidget = self.ui.sessionLogWidget
        self.ui.refSampleTabWidget.setColumnWidth (1, 250)

        self.ui.refSampleTabWidget.setHorizontalHeaderLabels (['Name','Value'])
        ...
        self.axisGroup.addButton(self.ui.xRadButton, 0)
        self.axisGroup.addButton(self.ui.yRadButton, 1)
        self.axisGroup.addButton(self.ui.zRadButton, 2)
        self.axisGroup.buttonClicked[int].connect(self.axisChecked)
        self.ui.boxOmitButton.clicked.connect(self.ui.openGLWidget.startSelectBox)
        timer = QTimer(self)
        ...
        self.ui.imtypeCB.setItemData (1, 0, Qt.UserRole -1)
        self.ui.imtypeCB.setItemData (2, 0, Qt.UserRole -1)
        ...
        self.fitarr = None
        str0 = '%f'%self.detector.dacopen
        self.ui.scan_dacopenLE.setText(str0)
    ...
    def getHome(self):
        ...
        self.paramFile ='%s\\atrex_params.txt'%(homedir)
        ...
        print 'Param file is : %s'%self.paramFile
        str0 = ''
        if (status):
            with open (self.paramFile) as f :
                ...
                if (len(str0) > 2):
                    self.ui.imDirLE.setText(str0)
                str0 = f.readline()
                if (len(str0) > 2):
                    self.ui.imfileLE.setText(str0)
                    self.imageFile = str0
                    print 'displaying ', str0
                    self.openImageFile(str0)
                str0 = f.readline()
                if (len(str0) > 2):
                    self.ui.outDirLE.setText(str0)
                str0 = f.readline()
                if (len(str0) > 2):
                    str0 = str0.strip()
                    self.detectFile = str0
                    self.detector.read_from_text_file(str0)
                    self.Display_Detector_calibration(self.detector)
                    self.myLogWidget.addSubEvent ('Read detector file : %s'%(str0))
        else :
    ...
    def closeEvent(self, event):
        print 'Shutting down....'
        qf = QFile(self.paramFile)
        qf.open(QIODevice.WriteOnly)
        qts = QTextStream(qf)
        if (len(self.imageDirectory) > 1):
    ...
    def openImage(self):
        ...
        self.ui.selectedImageLE.setText(str(mnmx[2]))
        self.ui.mergeStartLabel.setText (str(mnmx[0]))
        self.ui.mergeEndLabel.setText (str(mnmx[1]))
    ...
    def newSliderValue(self, newval):
        ...
        self.ui.selectedImageLE.setText(str(newval))
    ...
    def maxSliderUpdate(self, newval):
        str0 = '%f'%newval
        self.ui.imageMaxLE.setText(str0)
    ...
    def minSliderUpdate(self, newval):
        str0 = '%f' % newval
        self.ui.imageMinLE.setText(str0)
        maxval = self.ui.maxDNSlider.value()
        if (newval >= maxval) :
    ...
    def defImageDir(self):
        self.imageDirectory = QFileDialog.getExistingDirectory(self, 'Define Image Directory',
                                                                     self.imageDirectory,
    ...
    def defWorkDir(self):
        self.workDirectory = QFileDialog.getExistingDirectory(self, 'Define Work Directory',
                                                                    self.workDirectory,
                                                                    QFileDialog.ShowDirsOnly)
        # print self.imageDirectory
    ...
    def displayImage(self, filename):
        ...
        mn = float(self.ui.imageMinLE.text())
        mx = float(self.ui.imageMaxLE.text())


        self.imageWidget.setMinMax(mn, mx)
        ...
        if status:
            str0 = '0: %f   R: %f'%(self.myim.omega0, self.myim.omegaR)
            self.ui.omega0Lab.setText(str0)
            # self.ui.omegaRLab.setText (self.myim.omegaR)
            self.ui.chiLab.setText(str(self.myim.chi))
            self.ui.expLab.setText(str(self.myim.exposureT))
            self.ui.detectorLab.setText(str(self.myim.detector))
        # self.ui.imageWidget.writeQImage (self.myim.imArray)
    ...
    def newPeak(self, pt):
        ...
        newitem = QTableWidgetItem (str(pt.x()))
    ...
    def updatePeakList(self):
        ...
        #  lstr0 = QString(" %1\t%2").arg(x[i]).arg(y[i])
        #   self.ui.peakListWidget.addItem (lstr)
        ...
        #    xy = p.DetXY
        #    lstr0 = QString ("%1\t%2\t%3\t%4\t%5 ").arg(count).arg(p.HKL[0]).arg(p.HKL[1]).arg(p.HKL[2])
        #    #lstr0 = QString(" %1\t%2").arg(xy[0]).arg(xy[1])
        #    self.ui.peakListWidget.addItem(lstr)
    ...
    def peakListClicked(self, event):
        ...
        str0 = '%5.3f'%rms_std
        self.ui.peakRMSLE.setText(str0)
    ...
    def saveMask(self):
        outstr0 = QFileDialog.getSaveFileName(self, "Save Filename", self.workDirectory, 'Image File (*.tif)')
        self.mymask.saveToFile(outstr)

    def readMask(self):
        outstr0 = QFileDialog.getOpenFileName(self, "Save Filename", self.workDirectory, 'Image File (*.tif)')
    ...
    def updatePeakNumberLE(self):
        ...
        str0 = 'List1 : %d\tList 2 : %d'%(pn,pn1)
        self.ui.numPeaksLE.setText(str0)
    ...
    def PS(self, om, axis, prog):
        ...
        self.updatePeakList()
        self.ui.imageWidget.repaint()
        str0 = 'Number of peaks found : %d'%self.peaks.getpeakno()
        self.myLogWidget.addEvent (str0)
        msg = QMessageBox()
        msg.setIcon (QMessageBox.Information)
        msg.setInformativeText (str0)
        str0 = 'Image : %s'%self.imageFile
        #msg.setText (str0)
    ...
    def mergeImageRange(self):
        ...
        self.ui.imtypeCB.setItemData (2, 33, Qt.UserRole -1)
    ...
    def SavePeakTable(self):
        print 'write PT'
        wdir = self.ui.imDirLE.text()
        PTFile = QFileDialog.getSaveFileName(self, 'Save Peak Table', wdir)
        self.peaks.write_to_fileA(PTFile)

    def OpenPeakTable(self):
        print 'read PT'
        wdir = self.ui.imDirLE.text()
        PTFile = QFileDialog.getOpenFileName(self, 'Open Peak Table', wdir)
    ...
    def readTextDetect(self):
        detfile = QFileDialog.getOpenFileName(self, 'Open Detector File', self.workDirectory)
    ...
    def writeTextDetect(self):
        self.Update_Detector_calibration()
        detfile = QFileDialog.getSaveFileName(self, 'Output Detector File', self.workDirectory)
    ...
    def updatePlot(self):
        ...
        tstr0 = self.ui.plot_titleLE.text().toLatin1().data()
        xstr0 = self.ui.plot_xLE.text().toLatin1().data()
        ystr0 = self.ui.plot_yLE.text().toLatin1().data()
        self.myplotWidget.setLabels(tstr, xstr, ystr)
        self.myplotWidget.plotData()

    def savePlotToFile(self):
        filename = QFileDialog.getSaveFileName(self, 'Output Plot Image', self.workDirectory,
                                                     "Image File (*.png)")
        len = filename.length()
        if (len < 1): return
    ...
    def plotXYFromFile(self):
        ...
        filename = QFileDialog.getOpenFileName(self, 'ASCII XY File', self.workDirectory, "Text File (*.txt)")
        len = filename.length
        if (len < 1): return
    ...
    def readXPOW (self) :
        ...
        table.clearContents()
        filename = QFileDialog.getOpenFileName(self, "Input .txt File")
        refsamp = JCPDS ()
        ...
        for i in range(nrows) :
            ...
            itemTitle = QTableWidgetItem ()
            itemTitle.setBackgroundColor (Qt.yellow)
            itemValue = QTableWidgetItem ()
    ...
    def readJCPDS (self):
        table = self.ui.refSampleTabWidget
        table.clearContents()
        filename = QFileDialog.getOpenFileName(self, "Input .jcpds File")
        refsamp = JCPDS ()
    ...
    def cakeCurrent (self):
        ...
        if (self.ui.saveCakeCB.isChecked()) :
            saveFlag = True
            outfile = QFileDialog.getSaveFileName (None, "Cake OutputFile", self.workDirectory, "Tiff file (*.tif)")
        self.myim.cake (self.detector.getbeamXY(),self.detector.tthetaArr)
        #self.integratePlotWidget.setXYData_Integrate (self.myim.tthetabin, self.myim.avg2tth)

        self.displayCakeImage()
        str0 = QString ("%1").arg(int(self.myim.cakeParams[1]))

        self.ui.maxTTHLab.setText (str0)
        #Image Type combo box
        self.ui.imtypeCB.setItemData (1, 33, Qt.UserRole -1)
        ...
        if saveFlag :
            ...
            txtstr0 = QString("%1.txt").arg(outfile)
            tfile = file (txtstr.toLatin1().data(), 'w')
            str0 = "Cake Image (y: azimuth x: 2d-theta)\n"
            tfile.writelines(str0)
            str0 = "Source File : %s\n"%(self.myim.imFileName)
            tfile.writelines (str0)
            str0 = "Top Azimuth : -180."
    ...
    def peakFileBrowse (self) :
        str0 = QFileDialog.getSaveFileName (None, "Save to Peak Filename", self.workDirectory, "Any file (*.*);;Tiff file (*.tif)" )
        self.ui.peakSaveFilenameLE.setText(str0)
    ...
    def saveProj (self) :
        ...
        outfile = QFileDialog.getSaveFileName (self, "Project Settings File", basestring)
        qf = QFile (outfile)
        qf.open(QIODevice.WriteOnly)
        qts = QTextStream(qf)
    ...
    def readProj (self) :
        basestring = self.myproj.base+'projset.txt'
        pfile = QFileDialog.getOpenFileName (self, "Project Settings File", basestring)
        qf = QFile (pfile)
    ...
    def setValToControl (self, control, val) :
        #str0 = QString ("%1").arg(val)
        str0 ='%f'%val
        control.setText(str0)

    def axisChecked(self):
        ival = self.axisGroup.checkedId()
        print ival
